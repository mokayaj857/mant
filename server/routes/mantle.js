...
router.post('/mint-proof', async (req, res) => {
  try {
    ...
    // ethers.hashMessage(bytes32) reproduces "\x19Ethereum Signed Message:\n32" prefix.
    // signMessage will add the prefix and hash, matching toEthSignedMessageHash behavior
...
router.post('/checkin-proof', async (req, res) => {
  try {
    const { ticketId, eventId, account } = req.body;

    if (!ticketId || typeof ticketId !== 'string' && typeof ticketId !== 'number') {
      return res.status(400).json({ success: false, error: 'Missing required field: ticketId' });
    }
    ...
    if (!account || typeof account !== 'string') {
      return res.status(400).json({ success: false, error: 'Missing required field: account' });
    }
    ...
    const packedHash = ethers.solidityPackedKeccak256(
      ['string', 'uint256', 'uint256', 'address', 'uint256', 'uint256'],
      [action, BigInt(ticketId), BigInt(eventId), account, BigInt(timestamp), BigInt(nonce)]
    );
    ...
    console.error('Error creating Mantle checkin proof:', error);
    return res.status(500).json({ success: false, error: 'Failed to create checkin proof', details: error.message });
  }
});
